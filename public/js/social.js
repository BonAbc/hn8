...
function socialEditPost(postId) {
  ...
  if (trimmed.length > 15000) {
    alert("Your post is too long. Please keep it under 15000 characters.");
    return;
  }
...
const SOCIAL_MAX_CONTENT_LENGTH = 15000;
  ...
  socialPostCreateForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (content.length > SOCIAL_MAX_CONTENT_LENGTH) {
        throw new Error(
          "Your post is too long. Please keep it under 15000 characters.",
        );
      }
